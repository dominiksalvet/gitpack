...
exec 2>&1 1>/dev/null && # redirect streams of the following commands
...
echo log6 >&2 && test "$(grep -Fc '[exit]' ~/.local/share/gitpack/gitpack.log)" -eq 27 &&
...
echo log9 >&2 && test "$(grep -Fc '[paths]' ~/.local/share/gitpack/gitpack.log)" -eq 1 &&
