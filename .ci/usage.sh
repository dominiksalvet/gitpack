...
#-------------------------------------------------------------------------------
# DESCRIPTION:
#   This test checks whether GitPack works correctly during a basic usage. The
...
echo usage6 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
echo usage7 >&2 && echo "$gitpack_out" | grep '^\[install\]' &&
echo usage8 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
echo usage9 >&2 && echo "$gitpack_out" | grep '^\[installed\]' &&
echo usage10 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
...
echo usage14 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep)" &&
echo usage15 >&2 && echo "$gitpack_out" | grep '^\[older\]' &&
echo usage16 >&2 && rm ~/.local/share/gitpack/status && # should be recovered from a backup
echo usage17 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep)" &&
echo usage18 >&2 && echo "$gitpack_out" | grep '^\[update\]' &&
echo usage19 >&2 && rm ~/.local/share/gitpack/status &&
echo usage20 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep)" &&
echo usage21 >&2 && echo "$gitpack_out" | grep '^\[installed\]' &&
echo usage22 >&2 && rm ~/.local/share/gitpack/status &&
echo usage23 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep)" &&
...
echo usage29 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
echo usage30 >&2 && echo "$gitpack_out" | grep '^\[newer\]' &&
echo usage31 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
echo usage32 >&2 && echo "$gitpack_out" | grep '^\[downgrade\]' &&
echo usage33 >&2 && gitpack_out="$(gitpack install -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
echo usage34 >&2 && echo "$gitpack_out" | grep '^\[installed\]' &&
echo usage35 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep=2.1.0)" &&
...
# uninstall vhdldep
echo usage39 >&2 && gitpack_out="$(gitpack uninstall -o github.com/dominiksalvet/vhdldep)" &&
echo usage40 >&2 && echo "$gitpack_out" | grep '^\[uninstall\]' &&
echo usage41 >&2 && gitpack_out="$(gitpack uninstall -o github.com/dominiksalvet/vhdldep)" &&
echo usage42 >&2 && echo "$gitpack_out" | grep '^\[uninstalled\]' &&
echo usage43 >&2 && gitpack_out="$(gitpack status -o github.com/dominiksalvet/vhdldep)" &&
